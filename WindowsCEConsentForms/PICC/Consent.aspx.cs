    ...
    public partial class PICCConsent : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!isItNewSession)
                {
                    ...
                    ViewState["Signature1"] = formHandlerServiceClient.GetPatientSignature(patientId, "PICCConsent",
                                                                                           "signature1");
                    ViewState["Signature2"] = formHandlerServiceClient.GetPatientSignature(patientId, "PICCConsent",
                                                                                           "signature2");
                    ViewState["Signature3"] = formHandlerServiceClient.GetPatientSignature(patientId, "PICCConsent",
                                                                                           "signature3");
                    ViewState["Signature4"] = formHandlerServiceClient.GetPatientSignature(patientId, "PICCConsent",
                                                                                           "signature4");
                    ViewState["Signature5"] = formHandlerServiceClient.GetPatientSignature(patientId, "PICCConsent",
                                                                                           "signature5");
                }
            }
            catch (Exception)
        ...
        protected void BtnNext_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                bool result = formHandlerServiceClient.SavePatientSignature(patientId, Encoding.ASCII.GetString(bytes), "PICCConsent", "signature1");

                // updating signature2
                bytes = Encoding.ASCII.GetBytes(Request.Form["HdnImage2"]);
                result = formHandlerServiceClient.SavePatientSignature(patientId, Encoding.ASCII.GetString(bytes), "PICCConsent", "signature2");

                // updating signature3
                bytes = Encoding.ASCII.GetBytes(Request.Form["HdnImage3"]);
                result = formHandlerServiceClient.SavePatientSignature(patientId, Encoding.ASCII.GetString(bytes), "PICCConsent", "signature3");

                // updating signature4
                bytes = Encoding.ASCII.GetBytes(Request.Form["HdnImage4"]);
                result = formHandlerServiceClient.SavePatientSignature(patientId, Encoding.ASCII.GetString(bytes), "PICCConsent", "signature4");

                // updating signature4
                bytes = Encoding.ASCII.GetBytes(Request.Form["HdnImage5"]);
                result = formHandlerServiceClient.SavePatientSignature(patientId, Encoding.ASCII.GetString(bytes), "PICCConsent", "signature5");

                Session["NewSessionPICCConsent"] = false;

                Response.Redirect("/PICCConsentDeclaration.aspx");
            }
            catch (Exception)
            {
            }
